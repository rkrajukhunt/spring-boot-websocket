...
public class GroupServiceImpl implements GroupService {
  ...
  public GroupBean createGroup(GroupBean bean) throws Exception {
    ...
    User persistUser = userRepo.findById(bean.getCreatedBy()).orElseThrow(() -> new Exception("User not found !!!"));
  ...
  public List<GroupBean> getList(GroupBean bean) throws Exception {
    ...
    return rooms.stream().map(room -> {
      ...
      boolean check = room.getOccupants().stream().filter(o -> o.getUser().getId().equals(bean.getUserId())).findAny()
          .isPresent();
